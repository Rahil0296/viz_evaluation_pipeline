...
# Bars
plt.bar(x - width/2, legit_means, width,
        color='#0072B2',
        label=f'Legitimate (n={legit_count})')

plt.bar(x + width/2, fraud_means, width,
        color='#D55E00',
        label=f'Fraud (n={fraud_count})')

# Labels and formatting
plt.xticks(x, features, rotation=30, ha='right')
plt.ylabel('Average Transaction Metric Value')
plt.title('Ethereum Fraud Detection & Anomaly Detection: Mean Transaction Feature Comparison')
plt.legend()

plt.tight_layout()

# Save figure
plt.savefig('defi_anomalies_results/gpt5.2/run2_rich_context/viz1_rich_context_output.png', dpi=300)
plt.close()