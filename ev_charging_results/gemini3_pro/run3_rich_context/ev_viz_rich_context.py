...
# Load the dataset (handling potential path variations)
file_path = 'data/ev_charging_patterns.csv'
if not os.path.exists(file_path):
    file_path = 'ev_charging_patterns.csv'  # Fallback to current directory

df = pd.read_csv(file_path)

# Filter for specific features and drop missing values
...
plt.title("Correlation Heatmap of EV Charging Features and Energy Consumption", fontsize=16, pad=20)
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)

# Adjust layout and save the visualization
plt.tight_layout()
plt.savefig('ev_charging_results/gemini3_pro/run3_rich_context/ev_viz_rich_context_output.png', dpi=300, bbox_inches='tight')
plt.close()